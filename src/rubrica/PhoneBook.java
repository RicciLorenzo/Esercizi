...
public class PhoneBook extends View {
	...
	List<Entry> rubrica = new ArrayList<>();
	...
	public void add(String name, String surname, int phone, boolean sex) {
		...
		for(Entry contatto : rubrica) {
			if(rubrica.contains(name)&&rubrica.contains(surname)) {
				System.out.println("Nome e cognome giá presenti; sostituzione dei dati...");
				contatto.name=name;
				contatto.surname=surname;
				contatto.phone=phone;
				contatto.sex=sex;
			}			
		}
	...
	public void remove(String name, String surname) {
		...
		if(!(rubrica.contains(surname)&&rubrica.contains(name))) {
